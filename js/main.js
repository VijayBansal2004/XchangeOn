...
if($(".sidebar-right").length > 0){
  const steps = document.querySelectorAll(".sidebar-right ul li a");
  steps.forEach((step) => {
...
function updateCountdown(launchTime) {
  ...
  var x = setInterval(function () {
    ...
    $("#token-countdown-number-1").text(days);
    $("#token-countdown-number-2").text(hours);
    $("#token-countdown-number-3").text(minutes);
    $("#token-countdown-number-4").text(seconds);

    if ($("#connect-wallet-button").hasClass("disabled")) {
      // Do nothing
    } else {
    ...
    if (distance < 0) {
      ...
      $("#timer-container-1").html("<h1>Expired</h1>");
      $("#timer-container-2").html("<h1>Expired</h1>");
      $("#timer-container-3").html("<h1>Expired</h1>");
      $("#timer-container-4").html("<h1>Expired</h1>");
    }
  }, 1000);
}
updateCountdown("2025-12-20T12:00:00Z");
...
if ($("#chartdiv").length > 0) {
  am5.ready(function () {
    ...
    // Disable theme default colors
    let theme = am5themes_Animated.new(root);
    theme.rule("Series").setAll({
      stroke: am5.color("#08a66e"),
      fill: am5.color("#08a66e"),
    });
    ...
    let chart = root.container.children.push(
      am5xy.XYChart.new(root, {
        ...
        wheelY: "zoomX",
        pinchZoomX: true,
        paddingLeft: 0, // Remove chart-specific padding
        paddingRight: 0,
        paddingTop: 0,
        paddingBottom: 0,
      })
    ...
    let data = [
      { category: "Q4 23", value: 30 },
      { category: "Q1 24", value: 100 },
      { category: "Q2 24", value: 120 },
      { category: "Q3 24", value: 190 },
      { category: "Q4 24", value: 220 },
    ];
    ...
    // Set custom colors for strokes and fills
    series.strokes.template.setAll({
      stroke: am5.color("#08a66e"), // Orange stroke
      strokeWidth: 3,
    });

    series.fills.template.setAll({
      fill: am5.color("#08a66e"), // Orange fill
      fillOpacity: 0.3,
      visible: true,
    });
...
if ($("#donut-chart").length > 0) {
  am5.ready(function () {
    // Create root element
    var root2 = am5.Root.new("donut-chart");
    ...
    // Set orange color shades
    series.set(
      "colors",
      am5.ColorSet.new(root2, {
        colors: [
          am5.color("#08a66e"), // light orange
          am5.color("#06583b"), // medium orange
          am5.color("#07c783"), // dark orange
        ],
        reuse: true,
      })
    );
    ...
    // Set data
    series.data.setAll([
      { value: 40, category: "Ethereum" },
      { value: 30, category: "Solana" },
      { value: 30, category: "Arbitrum" },
    ]);
...
if ($("#chartnew").length > 0) {
  var chart = AmCharts.makeChart("chartnew", {
    ...
    color: "#fff",
    dataProvider: [
      { country: "ICO", value: 30 },
      { country: "Marketing Growth", value: 10 },
      { country: "Security Initiatives", value: 5 },
      { country: "Company Treasury", value: 25 },
      { country: "Equity Investors", value: 20 },
      { country: "Team And Advisors", value: 10 },
    ],
    valueField: "value",
    titleField: "country",
    outlineAlpha: 0.4,
    depth3D: 15,
    balloonText:
      "[[title]]<br><span style='font-size:10px'><b>[[value]]</b> ([[percents]]%)</span>", // Reduced font size
    angle: 30,
    colors: [
      "#06583b", // Dark orange
      "#08a66e", // Medium-dark orange
      "#07c783", // Bright orange
      "#0ae99a", // Lighter orange
      "#21c188", // Light orange
      "#47cf9e", // Pale orange
    ],
    labelRadius: 10, // Adjust label radius to fit smaller font
    labelText: "[[title]]: [[value]]", // Default label text
...
if ($(".integrated_slider").length > 0) {
  $(".integrated_slider").slick({
    slidesToShow: 4,
    slidesToScroll: 1,
    autoplay: true,
    autoplaySpeed: 0, // Set to 0 for immediate transition
    speed: 3000, // Duration of each slide transition in ms
    infinite: true,
    cssEase: "linear",
    arrows: false,
    responsive: [
...
if ($(".newslogo_slider").length > 0 || $(".newslogo_slider_3").length > 0) {
  $(".newslogo_slider, .newslogo_slider_3").slick({
    slidesToShow: 4,
    slidesToScroll: 1,
    autoplay: true,
    autoplaySpeed: 0, // Set to 0 for immediate transition
    speed: 5000, // Duration of each slide transition in ms
    infinite: true,
    cssEase: "linear",
    arrows: false,
    responsive: [
...
if ($(".newslogo_slider_2").length > 0 || $(".newslogo_slider_4").length > 0) {
  $(".newslogo_slider_2, .newslogo_slider_4").slick({
    rtl: false,
    slidesToShow: 4,
    slidesToScroll: 1,
    autoplay: true,
...
$(document).ready(function () {
  $("#roadmap-nav-date-pills .nav-link").click(function () {
    const selectedYear = $(this).text();
    const activeStatus = $("#roadmap-nav-status-pills .nav-link.active")
      .text()
      .toLowerCase()
      .replace(" ", "-");

    showBoxes(selectedYear, activeStatus);
  });

  // Status navigation
  $("#roadmap-nav-status-pills .nav-link").click(function () {
    const selectedStatus = $(this).text().toLowerCase().replace(" ", "-");
    const activeYear = $("#roadmap-nav-date-pills .nav-link.active").text();

    console.log(selectedStatus);
    console.log(activeYear);
    if (selectedStatus === "show all" || selectedStatus === "show-all") {
      $(".roadmap-box").show(); // Show all boxes if 'Show All' is selected
    } else {
      showBoxes(activeYear, selectedStatus);
    }
  });
  showBoxes();
});

function showBoxes(yearToShow = "2024", statusToShow = "show-all") {
  // Hide all roadmap boxes
  $(".roadmap-box").hide();

  if (statusToShow === "show all" || statusToShow === "show-all") {
    $(".roadmap-box." + yearToShow).show();
  } else {
    $(".roadmap-box." + yearToShow + "." + statusToShow).show();
  }
}

if($('pre').length > 0 || $('code').length > 0){
/* PrismJS 1.29.0
https://prismjs.com/download.html#themes=prism-tomorrow&languages=markup+css+clike+javascript+css-extras+java&plugins=line-highlight+line-numbers+highlight-keywords+command-line+normalize-whitespace+toolbar+copy-to-clipboard+treeview */
var _self =
